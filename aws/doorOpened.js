...
var mail_waiting_for_parcel = {
        ...
        Message: {
            Body: {
                Html: {
        Charset: 'UTF-8',
        Data:html_content_waiting
      },
            },
            Subject: {
                Data: "My Letterbox - Waiting Parcel"
            }
        },
...
var mail_door_open = {
    ...
    Message: {
        Body: {
            Html: {
    Charset: 'UTF-8',
    Data:html_content_door
  },},
        Subject: {
            Data: "My Letterbox - Door Opened"
        }
    },
    Source: "myletterbox.iot@gmail.com"
};
...
exports.handler = (event, context, callback) => {
    ...
    iotdata.getThingShadow(paramsGet, function (err, data) {
        ...
        else doorOpened(JSON.parse(data.payload), handlingSNS) // successful response
    })

};

//handle the increasing of letters
function doorOpened (dataJSON, callback){
        //Get if waiting for parcel
        var waitingForParcel = (dataJSON.state.desired.waitingForParcel == 'true');

        //payload send to the shadow
        var payloadUpdate = {
            "state": {"desired": {"doorOpened":true}}
        }
...
function handlingSNS(dataJSON){
    if(dataJSON.state.desired.waitingForParcel == true){
        var email = ses.sendEmail(mail_waiting_for_parcel, function(err, data){
        ...
        console.log('you were waiting for parcel and the door has been open,needs to send a SNS notification')
    }else{

        var email = ses.sendEmail(mail_door_open, function(err, data){
        if(err) console.log(err);
        else {
            console.log("===EMAIL SENT===");
            console.log(data);
        ...
        console.log('you were not waiting for parcel but the door has been open, needs to send a SNS notification')
    }
}
